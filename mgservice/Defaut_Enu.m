...
{
    UIImageView *image=[[UIImageView alloc]initWithFrame:CGRectMake(0,0,113,27)];
    image.image = [UIImage imageNamed:@"nav_titleimage.png"];
    [viewcontroller.navigationItem setTitleView:image];
//    [image release];
}
...
{
    UIImage *imagec = ImageWithView(view);
    CGImageRef ref = CGImageCreateWithImageInRect([imagec CGImage], rect);
...
void setnavigtionBackbar(UIViewController *viewcontroller)
{
    ...
    //backbutton.title = @"返回";
    backbutton.image = [UIImage imageNamed:@"navigation_backbuttonimage.png"];
...
{
    return [string sizeWithFont:[UIFont systemFontOfSize:font] constrainedToSize:CGSizeMake(width, 10000) lineBreakMode:NSLineBreakByWordWrapping].height;
}
//Label高度
float LabelheightWithStringF(NSString *string,UIFont *font,float width)
{
    return [string sizeWithFont:font constrainedToSize:CGSizeMake(width, 10000) lineBreakMode:NSLineBreakByWordWrapping].height;
}

//Label宽度
float LabelwidthWithString(NSString *string,float font,float height)
{
    return [string sizeWithFont:[UIFont systemFontOfSize:font] constrainedToSize:CGSizeMake(10000, height) lineBreakMode:NSLineBreakByWordWrapping].width;
}
//Label宽度
float LabelwidthWithStringF(NSString *string,UIFont *font,float height)
{
    return [string sizeWithFont:font constrainedToSize:CGSizeMake(10000, height) lineBreakMode:NSLineBreakByWordWrapping].width;
}
...
void moveFileResourceofType(NSString *filename,NSString *type)
{
    ...
    NSLog(@"path:%@",path);
    [[NSFileManager defaultManager] copyItemAtPath:path toPath:[Documents() stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.%@",filename,type]] error:nil];
}
...
SYSTEMLANGUAGESTATE systemlangyage()
{
    ...
    NSString* preferredLang = [languages objectAtIndex:0];//English,简体中文,繁体中文分别得到:en,zh-Hans,zh-Hant
    NSLog(@"preferredLang:%@",preferredLang);
    if ([preferredLang isEqualToString:@"en"])//English
    {
        return SYSTEMLANGUAGESTATEENGLISH;
    }
    
    if ([preferredLang isEqualToString:@"zh-Hans"])//简体中文
    {
        return SYSTEMLANGUAGESTATECHINA;
    }
    
    if ([preferredLang isEqualToString:@"zh-Hant"])//繁体中文
    {
        return SYSTEMLANGUAGESTATEOLDCHINA;
    }
    
    return SYSTEMLANGUAGESTATEOTHER;
}

//根据系统语言转变字体
NSString *stringbysystemlangyage(NSString *string)
{
    if (systemlangyage() == SYSTEMLANGUAGESTATECHINA) {
        return big5ToGb(string);
    }
    
    return gbToBig5(string);
}
...
{
    NSString *_string_GB = gbStringConvert();
    NSString *_string_BIG5 = big5StringConvert();
...
@implementation NSMutableAttributedString(EasySetting)

// 设置某段字的颜色
- (void)setColor:(UIColor *)color Range:(NSRange)range{
    if (range.location > self.length - 1 || range.length + range.location > self.length) {
        return;
    }
...
@implementation UIViewController (SHViewApi)
...
@implementation UIBarButtonItem(EasySetting)
...
+ (id)barButtonItemWithFrame:(CGRect)frame Image:(UIImage *)image Selected:(UIImage *)selectedImage Highlighted:(UIImage *)highlightedImage Target:(id)target Action:(SEL)action forControlEvents:(UIControlEvents)controlEvents
{
    ...
    [morebutton setImage:selectedImage forState:(UIControlStateSelected)];
    [morebutton setImage:highlightedImage forState:(UIControlStateHighlighted)];
    
    [morebutton addTarget:target action:action forControlEvents:controlEvents];
    
    UIBarButtonItem *morebar = [[UIBarButtonItem alloc] initWithCustomView:morebutton];
