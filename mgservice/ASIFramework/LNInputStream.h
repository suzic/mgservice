...
@interface LNInputStream : NSObject {
	NSInputStream *stream;
	LNHTTPRequest *request;
}
+ (id)inputStreamWithFileAtPath:(NSString *)path request:(LNHTTPRequest *)request;
+ (id)inputStreamWithData:(NSData *)data request:(LNHTTPRequest *)request;

@property (retain, nonatomic) NSInputStream *stream;
@property (assign, nonatomic) LNHTTPRequest *request;
@end
