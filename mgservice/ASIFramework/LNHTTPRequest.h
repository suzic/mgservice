...
#import "LNHTTPRequestDelegate.h"
#import "LNProgressDelegate.h"
#import "LNCacheDelegate.h"

@class LNDataDecompressor;

extern NSString *LNHTTPRequestVersion;
...
typedef enum _LNAuthenticationState {
	LNNoAuthenticationNeededYet = 0,
	LNHTTPAuthenticationNeeded = 1,
	LNProxyAuthenticationNeeded = 2
} LNAuthenticationState;

typedef enum _LNNetworkErrorType {
    LNConnectionFailureErrorType = 1,
    LNRequestTimedOutErrorType = 2,
    LNAuthenticationErrorType = 3,
    LNRequestCancelledErrorType = 4,
    LNUnableToCreateRequestErrorType = 5,
    LNInternalErrorWhileBuildingRequestType  = 6,
    LNInternalErrorWhileApplyingCredentialsType  = 7,
	LNFileManagementError = 8,
	LNTooMuchRedirectionErrorType = 9,
	LNUnhandledExceptionError = 10,
	LNCompressionError = 11
...
extern unsigned long const LNWWANBandwidthThrottleAmount;

#if NS_BLOCKS_AVAILABLE
typedef void (^LNBasicBlock)(void);
typedef void (^LNHeadersBlock)(NSDictionary *responseHeaders);
typedef void (^LNSizeBlock)(long long size);
typedef void (^LNProgressBlock)(unsigned long long size, unsigned long long total);
typedef void (^LNDataBlock)(NSData *data);
#endif

@interface LNHTTPRequest : NSOperation <NSCopying> {
	...
	id <LNHTTPRequestDelegate> delegate;
	...
	id <LNHTTPRequestDelegate, LNProgressDelegate> queue;
	...
	id <LNProgressDelegate> uploadProgressDelegate;
	...
	id <LNProgressDelegate> downloadProgressDelegate;
	...
	LNAuthenticationState authenticationNeeded;
	...
	LNDataDecompressor *dataDecompressor;
...
@property (assign) LNHTTPRequest *mainRequest;
...
@property (assign, readonly) LNAuthenticationState authenticationNeeded;
...
@property (assign) id <LNCacheDelegate> downloadCache;
@property (assign) LNCachePolicy cachePolicy;
@property (assign) LNCacheStoragePolicy cacheStoragePolicy;
...
#endif
@property (retain) LNDataDecompressor *dataDecompressor;
