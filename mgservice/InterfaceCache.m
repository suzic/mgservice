...
static InterfaceCache *sharedCache = nil;
static NSString *sessionCacheFolder = @"SessionStore";//临时
static NSString *permanentCacheFolder = @"PermanentStore";//永久
...
-(id)init
{
    ...
    [self setDefaultCachePolicy:LNUseDefaultCachePolicy];
    ...
    @synchronized(self)//保证此时没有其他线程对self对象进行修改
        {
            if(!sharedCache)
            {
                ...
                [sharedCache setStoragePath:[[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)objectAtIndex:0] stringByAppendingPathComponent:@"ASIHTTPRequestCache"]];//缓存路径
            }
        }
    }
    return sharedCache;
...
-(NSString *)storePath
{
    ...
    NSString *p = [[storePath retain]autorelease];
...
{
	[[self accessLock] lock];
	[self clearCachedResponsesForStoragePolicy:LNCacheForSessionDurationCacheStoragePolicy];
	[storagePath release];
...
-(void) updateExpiryForRequest:(LNHTTPRequest *)request maxAge:(NSTimeInterval)maxAge//更新过期数据
{
    NSString *headPath = [self pathToStoreCachedResponseDataForRequest:request];
    NSMutableDictionary *cachedHeaders = [NSMutableDictionary dictionaryWithContentsOfFile:headPath];
    if(!cachedHeaders)
    {
        return;
    }
    ...
    [cachedHeaders writeToFile:headPath atomically:NO];
}

-(NSDate *)expiryDateForRequest:(LNHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
    return [ASIFormDataRequest expiryDateForRequest:request maxAge:maxAge];
}
...
-(void)storeResponseForRequest:(LNHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if ([request error] || ![request responseHeaders] || ([request cachePolicy] & LNDoNotWriteToCacheCachePolicy)) {
		[[self accessLock] unlock];
		return;
	}
    ...
    NSMutableDictionary *responserHeaders = [NSMutableDictionary dictionaryWithDictionary:[request responseHeaders]];
    if ([request isResponseCompressed]) {
		[responserHeaders removeObjectForKey:@"Content-Encoding"];
	}
	...
	NSDate *expires = [self expiryDateForRequest:request maxAge:maxAge];
	if (expires) {
		[responserHeaders setObject:[NSNumber numberWithDouble:[expires timeIntervalSince1970]] forKey:@"X-ASIHTTPRequest-Expires"];
	}
	...
	[responserHeaders setObject:[NSNumber numberWithInt:statusCode] forKey:@"X-ASIHTTPRequest-Response-Status-Code"];
    
	[responserHeaders writeToFile:headerPath atomically:NO];
...
- (NSString *)pathToStoreCachedResponseDataForRequest:(LNHTTPRequest *)request
{
	...
	NSString *path = [[self storagePath] stringByAppendingPathComponent:([request cacheStoragePolicy] == LNCacheForSessionDurationCacheStoragePolicy ? sessionCacheFolder : permanentCacheFolder)];
...
{
	[[self accessLock] lock];
	LNCachePolicy cp = defaultCachePolicy;
	[[self accessLock] unlock];
	return cp;
}

//1
- (void)setDefaultCachePolicy:(LNCachePolicy)cachePolicy
{
	[[self accessLock] lock];
	if (!cachePolicy) {
		defaultCachePolicy = LNAskServerIfModifiedWhenStaleCachePolicy;
	}  else {
...
//3
- (void)clearCachedResponsesForStoragePolicy:(LNCacheStoragePolicy)storagePolicy
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
	...
	NSString *path = [[self storagePath] stringByAppendingPathComponent:(storagePolicy == LNCacheForSessionDurationCacheStoragePolicy ? sessionCacheFolder : permanentCacheFolder)];
...
+ (BOOL)serverAllowsResponseCachingForRequest:(LNHTTPRequest *)request
{
...
+ (NSString *)keyForURL:(NSURL *)url
{
    ...
    NSError *error = NULL;
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"phpsessid=[^&]*" options:NSRegularExpressionCaseInsensitive error:&error];
    NSString *modifiedurlString = [regex stringByReplacingMatchesInString:urlString options:0 range:NSMakeRange(0, [urlString length]) withTemplate:@""];
...
- (BOOL)canUseCachedDataForRequest:(LNHTTPRequest *)request
{
	...
	if ([request cachePolicy] & LNOnlyLoadIfNotCachedCachePolicy) {
		return YES;
        
        // If we want to fallback to the cache after an error
	} else if ([request complete] && [request cachePolicy] & LNFallbackToCacheIfLoadFailsCachePolicy) {
		return YES;
        
        // If we have cached data that is current, we can use it
	} else if ([request cachePolicy] & LNAskServerIfModifiedWhenStaleCachePolicy) {
	...
	} else if ([request cachePolicy] & LNAskServerIfModifiedCachePolicy) {
